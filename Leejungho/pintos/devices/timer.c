...
void timer_sleep (int64_t ticks) {
	...
	ASSERT (intr_get_level () == INTR_ON);
	// while (timer_elapsed (start) < ticks) //여기부터 수정
	// 	thread_yield ();
	...
	thread_block(); // 인터럽트 불러서 cpu 제어권넘기고 intr_off만들고 리스트에 넣기
	intr_set_level(old_level);
}
...
static void timer_interrupt (struct intr_frame *args UNUSED) { // 여기 수정해야됨
	ticks++;
	thread_tick ();
